...
class Adaboost:
		...
		def selectBest(self, featureVals, iis, labels, features, weights):
				...
				"""
				1. Create 'bestError' to store the error of the best weak classifier and
					'bestIndex' to store the index of it
				2. The first loop iterates through every weak classifier
				3. The second loop iterates through every integral image
				4. If featureVals[i][j] < 0, it means that the classifier identifies the image
					as a face. Otherwise, a non-face.
				5. If the image is identified wrongly, add its weight to 'error'
				6. Return the weak classifier with minimum error
				"""
				bestError = float('inf')
				bestIndex = -1
				for i in range(len(features)):
					...
					for j in range(len(iis)):
						isFace = (featureVals[i][j] < 0)
						error += weights[j] * (abs(isFace - labels[j]))
					if error < bestError:
						bestIndex = i
						bestError = error
				bestClf = WeakClassifier(features[bestIndex])
				...
				for i in range(len(features)):
					...
					if error < bestError:
						bestIndex = i
						bestError = error
				bestClf = WeakClassifier(features[bestIndex], threshold=-6)
